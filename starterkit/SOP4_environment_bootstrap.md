# SOP 4 — GhostCrab MCP : Vérification de l'environnement (Phase A)

**Version :** 1.0  
**Statut :** Stable  
**Périmètre :** Vérification complète d'une installation GhostCrab MCP — Docker, migrations, smoke test, connexion MCP client

---

## Prerequisites: GhostCrab MCP

This SOP assumes you have access to the **GhostCrab MCP server repository**. If you haven't cloned it yet:
...
**Full installation instructions, architecture details, and MCP tool reference:**  
→ [GhostCrab README](../README.md)

This SOP (Phase A) focuses exclusively on **environment verification**. For the conceptual model, ontology design, and tool usage patterns, refer to the main README and SOP1/SOP2.

> **Storage scope:** GhostCrab MCP targets **PostgreSQL only** (native extensions or fallback mode). SQLite is not supported in the current stack. Do not generate SQLite-based migration scripts or `INSERT OR REPLACE` statements.
...
## Checklist Phase A — Résumé

```
[ ] A1 — Prérequis système présents (Docker, Node.js, git)
[ ] A2 — Repo cloné et .env configuré
[ ] A3 — Stack PostgreSQL démarrée et healthy
[ ] A4 — Migrations appliquées
[ ] A5 — Smoke test passant
[ ] A6 — MCP client connecté (outils ghostcrab_* visibles)
[ ] A7 — ghostcrab_status retourne un état acceptable
```

**Signal de passage Phase A → Phase B :** tous les checkpoints ci-dessus cochés, `ghostcrab_status` sans erreur critique.
