# SOP 2 — Préparation du matériel ontologique depuis un vault Obsidian (pro-mcp)

**Edition:** pro-mcp only — `ghostcrab-mcp`, PostgreSQL, MCP **`ghostcrab_*`**, **mindCLI**, COPY bulk.

**Version :** 1.0  
**Statut :** Draft exploitable  
**Périmètre :** De la structure brute d'un vault Obsidian à l'injection validée dans GhostCrab Pro.

| Concept | Pro PostgreSQL |
|---------|----------------|
| Runtime | `ghostcrab-mcp` + Docker Postgres |
| CLI audit | `go run ../mindbot/cmd/mindcli --json mb_pragma ...` |
| Fact store | `mfo_facets`, `mfo_projections`, `graph.entity`, `graph.relation` |
| Bulk import | [SOP3](SOP3_parsing_pipeline.md) COPY, [SOP5](SOP5_source_import_compiler.md), [SOP6](SOP6_document_import.md) |
| MCP | Modélisation + requête + writes **unitaires** — jamais hot-path bulk |
| **Interdit** | `gcp brain structured-import` comme seul chemin bulk |
...
|---|---|---|
| Fichier `.md` | Entité + facettes | `mfo_facets` |
| PDF (section/page) | Source documentaire | `mfo_facets` |
| Tag Obsidian | Axe de facette | Facette indexable |
| `[[wikilink]]` | Edge de graphe | `graph.*` |
| Frontmatter YAML | Facettes structurées | `mfo_facets.facets` |
| Section logique | `source_ref` distinct | Unité d'extraction |
| Décision/tâche | Projection + entité typée | `mfo_projections` + Layer 1 |
...
> **Prérequis :** Phase B0 complétée — choix `linkml_or_sql` enregistré dans `../templates/import_path_choices.yaml` (voir [SOP0](SOP0_import_path_choices.md)). Ne pas utiliser cette section si l'utilisateur a choisi la voie MCP incrémentale seule (section 7 — unitaire uniquement).
...
Références canoniques : ontologies LinkML versionnées du workspace + `../templates/linkml_ontology.stub.yaml`.

### 6 bis.2 Séquence (Pro)

1. Compléter `jtbd.yaml` et obtenir confirmation du Model Proposal.
2. Copier `../templates/linkml_ontology.stub.yaml` vers `ontology/core.yaml`.
3. Le LLM étend le stub : classes, enums, slots, annotations Pro.
4. **Validation** : compile LinkML + revue DDL (`ghostcrab_ddl_propose` si tables Layer 1 requises).
5. **Import SQL** : migration approuvée via `ghostcrab_ddl_execute` ou pipeline SQL produit — pas `gcp brain ontology compile` seul.
6. Vérification : `ghostcrab_schema_list`, `ghostcrab_workspace_inspect`, `ghostcrab_coverage`.
7. Audit projections (recommandé, plus performant) :
   ```bash
...
## Section 7 — Voie A : injection MCP incrémentale (unitaire — Pro)

> **Prérequis :** Phase B0 — choix `mcp_incremental` dans `../templates/import_path_choices.yaml`. **Bulk vault** → [SOP3](SOP3_parsing_pipeline.md) COPY, pas cette section en volume.
...
**Pré-requis :** `workspace_id` unique, `backend` défini (**PostgreSQL** — la stack GhostCrab MCP de référence ne cible pas SQLite).
...
**Comportement upsert :** Si `source_ref` existe → mise à jour. Si absent → création. Basé sur `partial unique index` sur `source_ref` dans PostgreSQL.
