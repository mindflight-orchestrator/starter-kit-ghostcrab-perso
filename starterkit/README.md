...
Companion kit for [GhostCrab MCP](https://gitlab.com/webigniter/ghostcrab). Load this alongside a running GhostCrab MCP instance to model and ingest a project into PostgreSQL.
...
- Not a data migration tool (data ingestion goes through SQL direct — not MCP)
...
1. Clone this repo alongside `ghostcrab-mcp`:
   ```bash
   git clone <this-repo> ~/mindbrain-starterkit
   ```
...
Phase C — Parse and ingest files   →  SOP2 §7 + SOP3
```
